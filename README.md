...
Пайплайн настроен через GitHub Actions и запускается автоматически при каждом push в ветку `main`.
...
push в main
     ...
     ▼
┌─────────────┐  ┌─────────────┐  ┌─────────────┐
│ test-rust   │  │  test-go    │  │ test-python │  ← параллельно
└──────┬──────┘  └──────┬──────┘  └──────┬──────┘
       │                │                │
       ▼                ▼                ▼
┌─────────────┐  ┌─────────────┐  ┌─────────────┐
│ build-rust  │  │  build-go   │  │ build-python│  ← только если тесты прошли
└─────────────┘  └─────────────┘  └─────────────┘
       │                │                │
       └────────────────┴────────────────┘
                        │
...
**Тесты (параллельно для всех трёх сервисов):**
- Rust — `cargo test`
- Go — `go test ./... -v -race`
- Python — `ruff check` + `ruff format --check` + `pytest -v`

**Сборка и пуш (только если тесты прошли):**
- Собирает Docker-образ каждого сервиса
- Пушит на Docker Hub с тегом `latest`

## Образы на Docker Hub

После успешного запуска образы доступны по адресам:

```
твой_логин/rust:latest
твой_логин/go:latest
твой_логин/python:latest
```

## Настройка (один раз)
**1. Создать токен на Docker Hub:**
Docker Hub → Account Settings → Security → New Access Token
Скриншот выполнения:
![Создание токена](screenshots/ex_5_inc_dif_gen_token.jpg)
...
| `DOCKERHUB_TOKEN` | токен из шага 1 |
Скриншот выполнения:
![Github секреты](screenshots/ex_5_incr_dif_secrets.jpg)

**3. Запушить в main — пайплайн запустится автоматически.**
